...
def draw_network_graph(
    ...
    *,
    ...
    iterations=1200,
    spread=True,
    spread_factor=1.25,   # spreads around more but now bounded correctly
    # overlap resolver
    spread_pad=0.03,
    # visuals
    node_color="skyblue",
    ...
    # --- Figure/Axes FIRST (so fitting uses the final axes geometry) ---
    fig, ax = plt.subplots()
    ax.set_axis_off()
    ax.set_title(title)

    # Avoid tight_layout changing axes size after fitting (this caused out-of-bounds before)
    ...
    pos = nx.spring_layout(G, k=k, iterations=int(iterations), seed=seed)

    # Fit once (uses actual axes bbox including title/subplots_adjust)
