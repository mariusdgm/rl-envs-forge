...
def discrete_reward_env():
    """Fixture to create a CartPole environment with discrete reward."""
    return CartPole(continuous_reward=False)
...
class TestCartPole:
    ...
    def test_discrete_reward(self, discrete_reward_env):
        ...
        action = np.array([1.0], dtype=np.float32)  # Ensure action is of correct dtype
        discrete_reward_env.state = [0.0, 0.0, 0.0, 0.0]  # Set state for testing
        state, reward, done, truncated, info = discrete_reward_env.step(action)
        assert reward == 1.0  # In the upright position

        discrete_reward_env.state = [
            0.0,
            0.0,
            0.2,
            0.0,
        ]  # Set state outside of upright position
        state, reward, done, truncated, info = discrete_reward_env.step(action)
